{ config, pkgs, ... }:
let
  bgImageMiku = pkgs.stdenv.mkDerivation {
    name = "miku-image";
    src = ./img/miku.png;
    phases = [ "installPhase" ];
    installPhase = "cp $src $out";
  };
  bgImageFumo9 = pkgs.stdenv.mkDerivation {
    name = "fumo9-image";
    src = ./img/fumo9.png;
    phases = [ "installPhase" ];
    installPhase = "cp $src $out";
...
in
{
  home.file.".local/share/konsole/Miku.colorscheme".text = ''
    [Background]
    Color=35,38,39
    RandomHueRange=360
    ...
    FillStyle=Adapt
    Opacity=0.7
    Wallpaper=${bgImageMiku}
    WallpaperFlipType=NoFlip
    WallpaperOpacity=0.4
  '';

  home.file.".local/share/konsole/Fumo9.colorscheme".text = ''
    [Background]
    Color=35,38,39
    RandomHueRange=360
    RandomSaturationRange=100
